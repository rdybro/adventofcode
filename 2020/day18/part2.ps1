...
function Invoke-MathFunction($math) {
    ...
    while($math.ToCharArray() -contains "+") {

        $mathSplit = ($math.Split(" + ",2)[0..1] -join "+").Split(" ")

        for($i = 0; $i -lt $mathSplit.Count; $i++) {

            if($mathSplit[$i] -match "[0-9]+\+[0-9]+") { $mathSplit[$i] = Invoke-Expression $mathSplit[$i] }
        }
