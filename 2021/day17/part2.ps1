...
$possibleY = @([int]$targetY[0]..$([int]$targetY[0] * -1))

$peakInitialVelocity = $null
$peakY = 0

$possibleVelocityValues = @()
...
foreach($x in $possibleX) {
    ...
    foreach($y in $possibleY) {
        ...
        while(($positionX -le [int]$targetX[1]) -and ($positionY -ge [int]$targetY[0])) {
        ...
        if($hitTargetArea) { $possibleVelocityValues += "$x,$y" }
        if($hitTargetArea -and ($currentPeakY -gt $peakY)) { $peakInitialVelocity = "$x,$y"; $peakY = $currentPeakY }
    }
}

Write-Host "There are $($possibleVelocityValues.Count) distinct velocity values that cause the prope to hit the target area."
